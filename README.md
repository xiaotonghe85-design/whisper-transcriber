...
- 默认使用免费 LibreTranslate / Argos Translate 兼容接口，并在失败时自动切换到 MyMemory
- 页面直接展示结果
- 一键下载 `.txt`
- 提供 `/health` 健康检查
...
- `MYMEMORY_URL`
  - 默认值：`https://api.mymemory.translated.net/get`
- `MAX_UPLOAD_MB`
  - 默认值：`100`
  - 控制单个上传文件的大小上限
- `PORT`
...
- 当前版本支持 `.mp3`、`.m4a`、`.wav`
- 默认外语转中文先走免费 LibreTranslate / Argos Translate，失败后自动切到 MyMemory；公开免费实例可能会限流或临时不可用
